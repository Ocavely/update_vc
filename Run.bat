...
python -m pip install -r requirements.txt -f ./libs ^
    ...
    --trusted-host pypi.tuna.tsinghua.edu.cn >nul 2>&1
...
if defined WHL_FILE (
    if exist "!WHL_FILE!" (
        python -m pip install "!WHL_FILE!" --no-deps --force-reinstall >nul 2>&1
        if !errorlevel! equ 0 (
            echo "✅ wxautox4_wechatbot 安装成功"
        ) else (
            echo "❌ wxautox4_wechatbot 安装失败"
        )
    ) else (
