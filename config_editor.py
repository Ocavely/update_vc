...
# ===== 配置保存目录 =====
SAVE_CONFIG_DIR = 'save'
SAVE_CONFIG_FILE = 'save.config'

def get_save_config_path():
    """获取 save.config 文件路径"""
    script_dir = os.path.dirname(os.path.abspath(__file__))
    return os.path.join(script_dir, SAVE_CONFIG_DIR, SAVE_CONFIG_FILE)

def init_save_config():
    """初始化 save.config 文件，从 config.py 复制所有配置"""
    save_dir = os.path.dirname(get_save_config_path())
    ...
    try:
        ...
        app.logger.info(f"已初始化 save.config 文件")
    except Exception as e:
        app.logger.error(f"初始化 save.config 失败: {e}")
...
def merge_config_with_default(save_config, default_config):
    """合并配置，save.config 缺少的项从 default_config 补充"""
    merged = dict(default_config)
    merged.update(save_config)
    return merged
...
# 最近日志持久化存储
RECENT_LOGS_FILE = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'recent_logs.json')
recent_logs_list = []
MAX_RECENT_LOGS = 100  # 最多保存100条

# 加载历史日志
def load_recent_logs():
    global recent_logs_list
    try:
        if os.path.exists(RECENT_LOGS_FILE):
            with open(RECENT_LOGS_FILE, 'r', encoding='utf-8') as f:
                recent_logs_list = json.load(f)
                if not isinstance(recent_logs_list, list):
                    recent_logs_list = []
    except Exception as e:
        print(f"加载历史日志失败: {e}")
        recent_logs_list = []
...
# 保存日志到文件
def save_recent_logs():
    global recent_logs_list
    try:
        with open(RECENT_LOGS_FILE, 'w', encoding='utf-8') as f:
            json.dump(recent_logs_list, f, ensure_ascii=False, indent=2)
    except Exception as e:
        print(f"保存日志失败: {e}")
...
def bot_status():
    ...
    return {"status": current_status, "pid": current_bot_pid}
...
@app.route('/update-required')
def update_required():
    """更新提示页面"""
    current = request.args.get('current', 'v1.0')
    target = request.args.get('target', 'v2.0')
    return render_template('update_required.html', current_version=current, target_version=target)

# 远程日志 URL
REMOTE_UPDATE_LOG_URL = 'https://raw.giteeusercontent.com/ocavely_0/destiny/raw/master/lag'

@app.route('/api/update_log')
def get_update_log():
    """获取更新日志（优先从远程获取，失败则使用本地）"""
    # 优先尝试从远程获取
    try:
        import urllib.request
        req = urllib.request.Request(
            REMOTE_UPDATE_LOG_URL,
            headers={'User-Agent': 'Mozilla/5.0'}
        )
        with urllib.request.urlopen(req, timeout=10) as response:
            content = response.read().decode('utf-8')
            if content and content.strip():
                app.logger.info("从远程获取更新日志成功")
                return jsonify({'content': content})
    except Exception as e:
        app.logger.warning(f"从远程获取更新日志失败: {e}，尝试使用本地文件")

    # 远程失败，回退到本地文件
    try:
        log_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'lag')
        ...
        app.logger.error(f"读取更新日志失败: {e}")
        return jsonify({'content': ''}), 500

# 远程弹窗日志 URL（从Gitee获取）
REMOTE_WINDOWS_LOG_URL = 'https://raw.giteeusercontent.com/ocavely_0/destiny/raw/master/windows'
# 远程控制配置 URL
...
def sync_windowslag_from_remote():
    ...
    try:
        import urllib.request
        req = urllib.request.Request(
            REMOTE_WINDOWSLAG_URL,
            headers={'User-Agent': 'Mozilla/5.0'}
        )
        with urllib.request.urlopen(req, timeout=5) as response:
            remote_content = response.read().decode('utf-8').strip()
            if remote_content:
                # 解析远程配置
                remote_config = {}
...
def get_windowslag_value():
    """获取 windowslag 控制值（优先使用远程配置，保留本地其他字段）
    返回: (是否显示弹窗, 是否强制关闭, updateversion, nowversion)
    """
    # 尝试从远程同步（不修改本地文件）
    ...
    try:
        ...
        if os.path.exists(lag_file):
            ...
            for line in lines:
                ...
                if line_lower.startswith('closewindows') and '=' in line:
                    ...
                    parsed_content[line_lower.split('=')[0].strip()] = line.strip()

            # 如果 updateversion > nowversion，自动将 closewindows 设为 false
        ...
        # closewindows = true 强制关闭，不受 windowslag 影响
        if closewindows_enabled:
            return False, True, updateversion, nowversion
        return windowslag_enabled, False, updateversion, nowversion
    except Exception:
        return True, False, 0, 0

def set_windowslag_value(enabled):
    """设置 windowslag 控制值"""
    log_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'windowslog')
    lag_file = os.path.join(log_dir, 'windowslag.txt')
    try:
        os.makedirs(log_dir, exist_ok=True)
        with open(lag_file, 'w', encoding='utf-8') as f:
            f.write('windowslag = ' + ('true' if enabled else 'false'))
        return True
    except Exception as e:
        app.logger.error(f"设置 windowslag.txt 失败: {e}")
        return False

# 旧版本弹窗内容 URL
REMOTE_OLD_WINDOWS_LOG_URL = 'https://raw.giteeusercontent.com/ocavely_0/destiny/raw/master/oldwindowslag'

@app.route('/api/windows_log')
def get_windows_log():
    """获取 Windows 弹窗日志（优先从远程获取，失败则使用本地）"""
    # 检查控制开关
    show_popup, is_forced, updateversion, nowversion = get_windowslag_value()
    if not show_popup:
        if is_forced:
            app.logger.info("弹窗提醒已强制关闭（closewindows=true）")
        else:
    ...
    try:
        ...
        with urllib.request.urlopen(req, timeout=10) as response:
            ...
            app.logger.info(f"远程获取内容长度: {len(remote_content)}")
            if remote_content and remote_content.strip():
                app.logger.info("从远程获取弹窗日志成功")
                return jsonify({'content': remote_content})
            else:
                app.logger.warning("远程获取内容为空")
    except Exception as e:
        app.logger.warning(f"从远程获取弹窗日志失败: {e}，尝试使用本地文件")
    ...
    try:
        ...
        txt_files = [f for f in os.listdir(log_dir) if f.endswith('.txt') and f != 'windowslag.txt']
        if not txt_files:
            return jsonify({'content': ''})
        ...
        app.logger.error(f"读取 Windows 日志失败: {e}")
        return jsonify({'content': ''}), 500

@app.route('/api/dismiss_windows_popup', methods=['POST'])
def dismiss_windows_popup():
    """关闭弹窗提醒（设置closewindows=true）"""
    log_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'windowslog')
    lag_file = os.path.join(log_dir, 'windowslag.txt')
    try:
        os.makedirs(log_dir, exist_ok=True)
        ...
        # 只更新 closewindows 字段，保留其他
        lines = existing_content.strip().split('\n')
        new_lines = []
        closewindows_set = False
        for line in lines:
            if line.strip().startswith('closewindows'):
                new_lines.append('closewindows = true')
                closewindows_set = True
            elif line.strip():
                new_lines.append(line)
        if not closewindows_set:
            new_lines.append('closewindows = true')
        with open(lag_file, 'w', encoding='utf-8') as f:
            f.write('\n'.join(new_lines))
        return jsonify({'message': '弹窗提醒已关闭'})
    except Exception as e:
        app.logger.error(f"设置 closewindows 失败: {e}")
        return jsonify({'error': '设置失败'}), 500

@app.route('/api/set_closewindows', methods=['POST'])
def set_closewindows():
...
def get_system_info():
    ...
    try:
        ...
        # 获取临时记忆文件数量
        config = parse_config()
        memory_temp_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), config.get('MEMORY_TEMP_DIR', 'Memory_Temp'))
        temp_memory_count = 0
        if os.path.exists(memory_temp_dir):
            try:
                files = [f for f in os.listdir(memory_temp_dir) if f.endswith('.txt')]
                temp_memory_count = len(files)
            except:
                pass
        
        # 获取核心记忆文件数量
        core_memory_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), config.get('CORE_MEMORY_DIR', 'CoreMemory'))
        core_memory_count = 0
        if os.path.exists(core_memory_dir):
            try:
                files = [f for f in os.listdir(core_memory_dir) if f.endswith('.json')]
                core_memory_count = len(files)
            except:
                pass
        
        # 监听用户数
        listen_list = config.get('LISTEN_LIST', [])
        listen_user_count = len(listen_list)
        ...
        return jsonify({
            ...
            'uptime': f"{days * 24 + hours}时{minutes}分{seconds}秒",
            'chat_count': chat_count,
            'temp_memory_count': temp_memory_count,
            'core_memory_count': core_memory_count,
            'listen_user_count': listen_user_count
        })
    except Exception as e:
        app.logger.error(f"获取系统信息失败: {e}")
        return jsonify({'error': str(e)}), 500

@app.route('/api/feature_settings')
...
def toggle_feature():
    ...
    try:
        ...
        if not feature:
            return jsonify({'error': '未指定功能'}), 400
        
        config = parse_config()
        if feature not in config:
            return jsonify({'error': f'未知功能: {feature}'}), 400
        
        update_config({feature: value})
        app.logger.info(f"功能 {feature} 已设置为 {value}")
        
        return jsonify({'success': True})
    except Exception as e:
        app.logger.error(f"切换功能失败: {e}")
        return jsonify({'error': str(e)}), 500

@app.route('/api/update_emoji_prob', methods=['POST'])
def update_emoji_prob():
    """更新表情发送概率"""
    try:
        data = request.json
...
def save_all_config():
    ...
    try:
        ...
        # 更新功能开关
        for key in ['ENABLE_IMAGE_RECOGNITION', 'ENABLE_ASSISTANT_MODEL', 'ENABLE_IMAGE_GENERATION', 'ENABLE_ONLINE_API']:
            if key in data:
                new_values[key] = bool(data[key])
        
        # 更新表情概率
        if 'EMOJI_SENDING_PROBABILITY' in data:
...
def get_user_info():
    ...
    try:
        ...
        if os.path.exists(prompt_dir):
            prompt_files = [f[:-3] for f in os.listdir(prompt_dir) if f.endswith('.md')]
        ...
        response = jsonify({
            ...
            'active_user': chat_context_users[0] if chat_context_users else '--',
...
def get_recent_logs():
    ...
    try:
        # 检查是否需要清除日志
        clear_logs = request.args.get('clear', 'false').lower() == 'true'
        if clear_logs:
            recent_logs_list = []
            save_recent_logs()
            return jsonify({'logs': [{'time': '--:--:--', 'level': 'info', 'message': '日志已清除'}], 'cleared': True})
        
        logs = []
        # 从持久化列表获取最近的日志（最后20条）
        recent_logs = recent_logs_list[-20:] if len(recent_logs_list) > 20 else recent_logs_list
        ...
        for line in recent_logs:
            ...
            if line:
                ...
                parts = line.split(' - ', 2)
                if len(parts) >= 3:
                    time_part = parts[0].split('.')[0] if '.' in parts[0] else parts[0]
                    level = 'info'
                    if 'ERROR' in parts[1]:
                        level = 'error'
                    elif 'WARNING' in parts[1]:
                        level = 'warning'
                    logs.append({'time': time_part, 'level': level, 'message': parts[2]})
                else:
                    logs.append({'time': '--:--:--', 'level': 'info', 'message': line})
        
        # 如果没有日志，返回默认提示
        if not logs:
            logs = [{'time': '--:--:--', 'level': 'info', 'message': '等待接收日志...'}]
        
        return jsonify({'logs': logs})
    except Exception as e:
        app.logger.error(f"获取最近日志失败: {str(e)}")
        return jsonify({'logs': [{'time': '--:--:--', 'level': 'info', 'message': '日志加载失败'}]})

@app.route('/api/check_wechat', methods=['POST'])
def check_wechat():
    """检测微信是否运行"""
    try:
        ...
        return jsonify({'running': wechat_running, 'version': version})
    except Exception as e:
...
def submit_config():
    ...
    try:
        ...
        # 处理API Key字段的特殊逻辑
        api_key_fields = ['DEEPSEEK_API_KEY', 'MOONSHOT_API_KEY', 'ONLINE_API_KEY', 'ASSISTANT_API_KEY', 'FORUM_API_KEY', 'IMAGE_GENERATION_API_KEY']
        ...
        boolean_fields = [
            ...
            'ENABLE_TEXT_COMMANDS', 'ENABLE_GPT_SOVITS_TTS',
            'ENABLE_IMAGE_GENERATION', 'ENABLE_ASSISTANT_CUSTOM_PROMPT',
            'ENABLE_SELFIE_MODE', 'USE_ASSISTANT_FOR_IMAGE_GENERATION',
            'IMAGE_DETECTION_USE_ASSISTANT', 'ENABLE_EMOTION_FIX'
        ]
...
def parse_config():
    """读取配置，优先从 save.config，缺少的字段从 config.py 补充"""
    save_file = get_save_config_path()

    # 如果 save.config 不存在，先初始化
    if not os.path.exists(save_file):
        init_save_config()

    # 优先从 save.config 读取
    save_config = parse_config_from_file(save_file)

    # 从 config.py 读取默认配置
    script_dir = os.path.dirname(os.path.abspath(__file__))
    config_path = os.path.join(script_dir, 'config.py')
    default_config = parse_config_from_file(config_path)

    # 合并配置
    config = merge_config_with_default(save_config, default_config)
    return config
...
def update_config(new_values):
    ...
    with tempfile.NamedTemporaryFile('w', delete=False, dir=save_dir, encoding='utf-8') as temp_file:
        temp_file_name = temp_file.name
        temp_file.writelines(new_lines)

    # 替换原配置文件
    shutil.move(temp_file_name, save_file)
    app.logger.info(f"配置已保存到 save/save.config")
...
def quick_start():
    if request.method == 'POST':
        try:
            ...
            update_config(new_values)
            # 添加时间戳参数确保浏览器加载最新配置
            return redirect(url_for('config_editor') + '?t=' + str(int(time.time() * 1000)))
        except Exception as e:
...
'IGNORE_GROUP_CHAT_FOR_AUTO_MESSAGE', 'ENABLE_SENSITIVE_CONTENT_CLEARING', 'SAVE_MEMORY_TO_SEPARATE_FILE', 'ENABLE_INTERRUPT_REPLY',
                'ENABLE_IMAGE_GENERATION', 'ENABLE_ASSISTANT_CUSTOM_PROMPT',
                'ENABLE_SELFIE_MODE', 'ENABLE_EMOTION_FIX'
            ]
            ...
            return redirect(url_for('config_editor')) # 保存后重定向到自身以刷新GET请求
        except Exception as e:
...
def delete_reminder():
    try:
        data = request.get_json()
        reminder_index = data.get('index')
        
        json_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'recurring_reminders.json')
        if not os.path.exists(json_path):
            return jsonify({'error': '提醒文件不存在'}), 404
        ...
        if reminder_index is None or reminder_index < 0 or reminder_index >= len(reminders):
            return jsonify({'error': '无效的索引'}), 400
        
        deleted = reminders.pop(reminder_index)
        ...
        if 'logs' in request.json:  # 批量日志
            ...
            if isinstance(logs_data, list):
                for log_entry in logs_data:
                    if log_entry:
                        log_entry_clean = log_entry.strip()
                        colored_log = f"[BOT] {log_entry_clean}"
                        log_queue.put(colored_log)
                        # 保存到持久化列表
                        recent_logs_list.append(log_entry_clean)
                        if len(recent_logs_list) > MAX_RECENT_LOGS:
                            recent_logs_list = recent_logs_list[-MAX_RECENT_LOGS:]
                # 保存到文件
                save_recent_logs()
            ...
            if log_data:
                log_data_clean = log_data.strip()
                colored_log = f"[BOT] {log_data_clean}"
                log_queue.put(colored_log)
                # 保存到持久化列表
                recent_logs_list.append(log_data_clean)
                if len(recent_logs_list) > MAX_RECENT_LOGS:
                    recent_logs_list = recent_logs_list[-MAX_RECENT_LOGS:]
                save_recent_logs()
...
def test_tts():
    ...
    try:
        data = request.get_json()
        test_text = data.get('text', '你好，这是一段测试语音')
        ...
        # 强制重新读取config.py文件内容
        cfg_file_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'config.py')
        with open(cfg_file_path, 'r', encoding='utf-8') as f:
            cfg_content = f.read()
            import re
            match = re.search(r'ENABLE_GPT_SOVITS_TTS\s*=\s*(\w+)', cfg_content)
            if match:
                cfg.ENABLE_GPT_SOVITS_TTS = (match.group(1) == 'True')
                print(f"[TTS Test] 从文件读取 ENABLE_GPT_SOVITS_TTS = {cfg.ENABLE_GPT_SOVITS_TTS}")

        print(f"[TTS Test] ENABLE_GPT_SOVITS_TTS = {cfg.ENABLE_GPT_SOVITS_TTS}")
        print(f"[TTS Test] GPT_SOVITS_API_URL = {cfg.GPT_SOVITS_API_URL}")

        if not cfg.ENABLE_GPT_SOVITS_TTS:
            return jsonify({'status': 'error', 'message': f'GPT-SoVITS TTS功能未启用 (当前值: {cfg.ENABLE_GPT_SOVITS_TTS})'})
        ...
        # 使用当前配置创建TTS服务
        tts = GPTSovitsTTS()
        tts.tts_api_url = cfg.GPT_SOVITS_API_URL
        tts.set_gpt_url = cfg.GPT_SOVITS_SET_GPT_URL
        tts.set_sovits_url = cfg.GPT_SOVITS_SET_SOVITS_URL
        tts.gpt_weights_path = cfg.GPT_SOVITS_GPT_WEIGHTS
        tts.sovits_weights_path = cfg.GPT_SOVITS_SOVITS_WEIGHTS
        tts.ref_audio_path = cfg.GPT_SOVITS_REF_AUDIO
        tts.prompt_text = cfg.GPT_SOVITS_PROMPT_TEXT
        tts.top_k = cfg.GPT_SOVITS_TOP_K
        tts.top_p = cfg.GPT_SOVITS_TOP_P
        tts.temperature = cfg.GPT_SOVITS_TEMPERATURE
        tts.speed_factor = cfg.GPT_SOVITS_SPEED_FACTOR
        tts.repetition_penalty = cfg.GPT_SOVITS_REPETITION_PENALTY
        tts.sample_steps = cfg.GPT_SOVITS_SAMPLE_STEPS
        tts._initialized = False

        # 检查API是否可用
        if not tts.check_api_available():
            return jsonify({'status': 'error', 'message': 'GPT-SoVITS API服务未启动，请先运行 api_v2.py'})
        ...
        if audio_path and os.path.exists(audio_path):
            ...
            return jsonify({'status': 'error', 'message': '语音生成失败'})

    except Exception as e:
        import traceback
        return jsonify({'status': 'error', 'message': f'测试失败: {str(e)}', 'trace': traceback.format_exc()})
...
if __name__ == '__main__':
    ...
    print("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━")
    print("")
    print(f"📡 访问地址: http://localhost:{config.get('PORT', '5000')}/")
    print("   → 浏览器将自动打开，如未打开请手动访问上述地址")
    if config.get('ENABLE_LOGIN_PASSWORD', False):
        print("")
        print(f"🔐 登录密码: {config.get('LOGIN_PASSWORD', '未设置')}")
        print("   → 请妥善保管，勿泄露给他人")
    print("")
    print("💡 常见问题：")
    print("   问题1: 点击Start Bot没反应，提示微信窗口找不到")
    print("        → 解决: 重启一次微信")
    print("")
    print("   问题2: 导入配置后出现404心跳报错日志")
    print("        → 解决: 重启一次Run.bat")
    print("")
    print("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━")
