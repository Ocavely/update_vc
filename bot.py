...
async def async_delay():
    ...
    await asyncio.sleep(7)  # 异步延迟7秒，不阻塞其他任务
...
current_processing_user = None  # 当前正在处理的用户ID
processing_start_time = 0  # 开始处理的时间
INTERRUPT_CHECK_INTERVAL = 0.5  # 检查间隔（秒）
is_interrupted = False  # 是否被中断
is_handling_reminder = False  # 是否正在处理提醒（添加/删除）操作
...
# 获取微信窗口对象
try:
    logger.info("正在初始化微信接口...")
    wx = WeChat()
    logger.info("微信接口初始化成功！")
except Exception as e:
    logger.error("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━")
    logger.error("初始化微信接口失败！")
    logger.error(f"错误: {e}")
    logger.error("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━")
    logger.error("")
    logger.error("📋 解决方案：")
    logger.error("")
    logger.error("方案1: 重启微信")
    logger.error("   → 适用场景: 出现'NoneType'或'窗口未找到'错误")
    logger.error("   → 操作步骤: 完全退出微信 → 重新打开 → 登录后再运行")
    logger.error('')
    logger.error("方案2: 检查微信版本")
    logger.error("   → 可能版本不兼容")
    logger.error("   → 要求版本: 微信4.1.2")
    logger.error('')
    logger.error("")
    logger.error("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━")
    exit(1)
...
last_image_gen_time = 0.0 # 上一次图片生成时间
...
def parse_time(time_str):
    try:
        ...
        logger.error("错误：主动消息安静时间设置有误！请填00:00-23:59 不要填24:00,并请注意中间的符号为英文冒号！")
...
# 修改get_user_prompt函数
def load_emotion_fix_prompt():
    """加载情感优化提示词"""
    try:
        emotion_fix_path = os.path.join(root_dir, f'{EMOTION_FIX_FILE}.md')
        if os.path.exists(emotion_fix_path):
            with open(emotion_fix_path, 'r', encoding='utf-8') as f:
                return f.read()
        else:
            logger.warning(f"情感优化提示词文件不存在: {emotion_fix_path}")
            return None
    except Exception as e:
        logger.error(f"加载情感优化提示词失败: {e}")
        return None
...
def get_deepseek_response(message, user_id, store_context=True, is_summary=False):
    ...
    try:
        ...
        if store_context:
            ...
            # 2. 检查是否启用情感优化功能，如果启用则动态插入情感优化提示词
            if get_dynamic_config('ENABLE_EMOTION_FIX', ENABLE_EMOTION_FIX):
                emotion_fix_content = load_emotion_fix_prompt()
                if emotion_fix_content:
                    messages_to_send.append({"role": "system", "content": emotion_fix_content})
                    logger.debug(f"已为用户 {user_id} 动态插入情感优化提示词")
    ...
    while attempt <= max_retries:
        try:
            ...
            if "real name verification" in error_info:
                ...
                logger.error("错误：API 服务商反馈，你的余额不足，请先充值再使用! 如有余额，请检查令牌是否为无限额度。")
                break  # 终止循环，不再重试
                ...
                logger.error("错误：Prompt或消息中含有敏感词，无法生成回复，请清理临时记忆！")
    ...
    while attempt <= max_retries:
        try:
            ...
            response = assistant_client.chat.completions.create(
                ...
                stream=False,
                # 禁用工具调用，防止辅助模型（如gpt-4o-mini）自动触发画图等工具
                tool_choice="none",
                tools=[]
            )
            ...
            if "real name verification" in error_info:
                ...
                logger.error("错误：提示词中含有敏感词，无法生成回复，请联系API服务商！")
                if ENABLE_SENSITIVE_CONTENT_CLEARING:
                    logger.warning(f"已开启敏感词自动清除上下文功能，开始清除用户 {user_id} 的聊天上下文和临时记忆")
                    clear_chat_context(user_id)
                    clear_memory_temp_files(user_id)  # 清除临时记忆文件
...
def message_listener(msg, chat):
    global can_send_messages, last_generated_image_path
    who = chat.who 
    msgtype = msg.type
...
def recognize_image_with_moonshot(image_path, is_emoji=False):
    ...
    try:
        ...
        if not os.path.exists(processed_image_path):
            logger.error(f"图片文件不存在: {processed_image_path}")
            can_send_messages = True
            return ""
        
        file_size = os.path.getsize(processed_image_path)
        if file_size < 100:
            logger.warning(f"图片文件太小({file_size}字节)，可能无法识别: {processed_image_path}")
            can_send_messages = True
            return "[图片]" if not is_emoji else "[表情]"
        ...
        logger.debug(f"正在调用图片识别API，图片大小: {file_size} 字节")
        response = requests.post(f"{MOONSHOT_BASE_URL}/chat/completions", headers=headers, json=data, timeout=30)
        response.raise_for_status()
        result = response.json()
        
        if 'choices' not in result or len(result['choices']) == 0:
            logger.error(f"API返回结果格式异常: {result}")
            can_send_messages = True
            return "[图片]" if not is_emoji else "[表情]"
        
        recognized_text = result['choices'][0]['message']['content']
        
        error_keywords = ["请上传图片", "无法识别", "无法处理", "上传图片", "image_url"]
        if any(keyword in recognized_text for keyword in error_keywords):
            logger.warning(f"图片识别返回错误结果: {recognized_text}")
            can_send_messages = True
            return "[图片]" if not is_emoji else "[表情]"
        ...
        logger.error("图片识别API超时")
        can_send_messages = True
        return "[图片]" if not is_emoji else "[表情]"
    except requests.exceptions.RequestException as e:
        logger.error(f"图片识别API请求失败: {str(e)}")
        can_send_messages = True
        return "[图片]" if not is_emoji else "[表情]"
    except Exception as e:
...
def handle_wxauto_message(msg, who):
    ...
    try:
        # 每小时清理一次过期图片
        if time.time() - last_image_gen_time > 3600:
            clean_old_generated_images()
        ...
        # --- 1. 提醒检查 (包含添加和删除) ---
        if ENABLE_REMINDERS:
            reminder_result = try_handle_reminder_request(original_content, username)
            if reminder_result:
                global is_handling_reminder
                is_handling_reminder = False
                return
        ...
        if ENABLE_IMAGE_GENERATION:
            ...
            # 优先使用主模型判断是否需要画图（如果开启）
            if USE_ASSISTANT_FOR_IMAGE_GENERATION:
                current_time = time.time()
                if current_time - last_image_gen_time < 10:
                    logger.info(f"图片生成冷却中，跳过画图检测")
                elif image_generation_in_progress:
                    logger.info(f"图片生成进行中，跳过画图检测")
                else:
                    try:
                        result = needs_image_generation(content, username)
                        if isinstance(result, tuple):
                            image_type, image_prompt = result
                        else:
                            image_type = "不需要画图"
                            image_prompt = None
                        ...
                        if image_type == "不需要画图":
                            ...
                            logger.info(f"辅助模型判断需要自拍，用户 {username}，合并用户消息和描述生成图片")
                            reply_text = '我现在就去拍，等我一会...'
                            send_reply(username, username, username, '[图片生成]', reply_text, is_system_message=True)

                            if USER_SELFIE_DESCRIPTION:
                                combined_prompt = f"{content}: {USER_SELFIE_DESCRIPTION.strip()}"
                                logger.info(f"自拍模式触发，使用合并提示词: {combined_prompt[:50]}...")
                                image_path = generate_image(combined_prompt)
                            else:
                                logger.warning("自拍模式未设置AI形象描述")
                                image_path = None
                            if image_path:
                                last_generated_image_path = image_path
                                success = send_generated_image(username, image_path)
                                reply_text = '我拍好啦~' if success else '自拍生成成功，但发送失败，请稍后再试。'
                            else:
                                reply_text = '自拍生成失败，请稍后再试。'
                            send_reply(username, username, username, '[图片生成]', reply_text, is_system_message=True)
                            ...
                            last_image_gen_time = current_time
                            image_generation_in_progress = True
                            logger.info(f"辅助模型判断需要画图，用户 {username}，使用消息原文作为提示词")
                            reply_text = '我现在就去画，等我一会...'
                            send_reply(username, username, username, '[图片生成]', reply_text, is_system_message=True)
                            ...
                            if image_path:
                                last_generated_image_path = image_path
                                success = send_generated_image(username, image_path)
                                reply_text = '我画好啦~' if success else '图片生成成功，但发送失败，请稍后再试。'
                            else:
                        ...
                        logger.error(f"画图检测失败，回退到关键词检测: {e}")
                        # 回退到下面的关键词检测逻辑
            else:
                # --- 关键词匹配模式（原有逻辑） ---

                # 优先检测自拍关键词（支持多个，用/分割）
                if ENABLE_SELFIE_MODE and SELFIE_KEYWORD:
                    selfie_keywords = [k.strip() for k in SELFIE_KEYWORD.split('/') if k.strip()]
                    matched_selfie_keyword = None
                    for keyword in selfie_keywords:
                        if keyword in content:
                            matched_selfie_keyword = keyword
                            break

                    if matched_selfie_keyword:
                        logger.info(f"检测到自拍关键词 '{matched_selfie_keyword}'，用户 {username}: {original_content}")
                        current_time = time.time()
                        if current_time - last_image_gen_time < 10:
                            logger.info(f"图片生成冷却中，忽略自拍关键词检测")
                        elif image_generation_in_progress:
                            logger.info(f"图片生成进行中，忽略自拍关键词检测")
                        else:
                            last_image_gen_time = current_time
                            image_generation_in_progress = True
                            if USER_SELFIE_DESCRIPTION:
                ...
                # 检测普通图片生成关键词
                if IMAGE_GENERATION_KEYWORD:
                    keywords = [k.strip() for k in IMAGE_GENERATION_KEYWORD.split('/') if k.strip()]
                    matched_keyword = None
                    matched_pos = float('inf')
                    for keyword in keywords:
                        pos = content.find(keyword)
                        if pos != -1 and pos < matched_pos:
                    ...
                    if matched_keyword:
                        current_time = time.time()
                        if current_time - last_image_gen_time < 10:
                            logger.info(f"图片生成冷却中，忽略关键词检测")
                        elif image_generation_in_progress:
                            logger.info(f"图片生成进行中，忽略关键词检测")
                        else:
                            last_image_gen_time = current_time
                            image_generation_in_progress = True
                            logger.info(f"检测到图片生成关键词 '{matched_keyword}'，用户 {username}: {original_content}")
                            try:
                                image_prompt = content[matched_pos + len(matched_keyword):].strip()
                                if not image_prompt:
                                    reply_text = f'请在关键词"{matched_keyword}"后提供图片描述，例如: {matched_keyword} 一个可爱的女孩'
                                    send_reply(username, username, username, '[图片生成]', reply_text, is_system_message=True)
                                    image_generation_in_progress = False
                                    return
                                ...
                                logger.error(f"处理图片生成失败: {e}")
                                error_text = '图片生成失败，请稍后再试。'
                                send_reply(username, username, username, '[图片生成]', error_text, is_system_message=True)
                                image_generation_in_progress = False
                                return
        ...
        if msg.type in ('image'):
            if ENABLE_IMAGE_RECOGNITION:
                ...
                if img_path:
                    # 检查是否是刚刚生成的图片，如果是则跳过识别
                    if last_generated_image_path and str(img_path) == str(last_generated_image_path):
                        logger.info(f"检测到刚生成的图片，跳过图片识别: {img_path}")
                        last_generated_image_path = None  # 重置，防止后续干扰
                        img_path = None  # 清空路径，阻止后续识别
                    else:
                        is_emoji = False
                    ...
                    logger.error("⚠️ 图片识别功能异常，请查看解决方案：https://s.apifox.cn/b2f07354-bce7-4959-a803-97ed82c508ff/7649190m0")
            else:
        ...
        # --- 5. 将最终处理后的消息加入队列或直接处理（打断功能） ---
        # 只有在 processed_content 有效时才加入队列
        if processed_content:
            current_time_str = datetime.now().strftime("%Y-%m-%d %A %H:%M:%S")
            content_with_time = f"[{current_time_str}] {processed_content}"
            logger.info(f"准备将处理后的消息加入队列 - 用户 {username}: {content_with_time[:150]}...")

            sender_name = username
            enable_interrupt = get_dynamic_config('ENABLE_INTERRUPT_REPLY', True)

            # 检查是否需要打断当前回复
            if enable_interrupt:
                with interrupt_reply_lock:
                    is_current_user = (current_processing_user == username)
                    is_processing = is_current_user and (is_sending_message or processing_start_time > 0)

                # 打断条件：当前正在处理该用户的消息（无论AI思考还是发送阶段）
                # 排除：图片生成中、提醒操作中
                if is_current_user and (current_processing_user == username):
                    # 检查是否有正在进行的特殊操作（不打断这些操作）
                    if image_generation_in_progress:
                        logger.info(f"图片生成进行中，暂不打断，用户 {username}")
                        # 加入队列，等待当前操作完成
                        pass
                    elif is_handling_reminder:
                        logger.info(f"提醒操作进行中，暂不打断，用户 {username}")
                        # 加入队列，等待当前操作完成
                        pass
                    else:
                        logger.info(f"检测到新消息，打断当前回复（AI思考或发送中），用户 {username}")
                        # 设置中断标志
                        with interrupt_reply_lock:
                        ...
                        with queue_lock:
                            if username in user_queues:
                                user_queues[username]['messages'] = []
                        # 跳过队列，直接处理新消息
                        process_user_messages_direct(username, content_with_time, sender_name)
                        return
...
def process_user_messages_direct(user_id, single_message, sender_name):
    """直接处理单条消息（用于打断功能），不经过队列"""
    global can_send_messages, current_processing_user, processing_start_time, is_interrupted
    ...
    logger.info(f"直接处理用户 {user_id} 的新消息: {single_message[:50]}...")
    
    try:
        # 检测是否需要联网
        online_info = None
        if ENABLE_ONLINE_API:
            search_content = needs_online_search(single_message, user_id)
            if search_content:
                merged = f"用户原始信息：\n{single_message}\n\n需要进行联网搜索的信息：\n{search_content}"
                online_info = get_online_model_response(merged, user_id)
                if online_info:
                    final_prompt = f"""
用户的原始问题是：
"{single_message}"
...
请结合你的角色设定，以自然的方式回答用户的原始问题。
"""
                    reply = get_deepseek_response(final_prompt, user_id, store_context=True)
                else:
                    reply = get_deepseek_response(single_message, user_id, store_context=True)
            else:
                reply = get_deepseek_response(single_message, user_id, store_context=True)
        else:
            reply = get_deepseek_response(single_message, user_id, store_context=True)
        ...
        if reply:
            ...
            if "## 记忆片段" not in reply:
                send_reply(user_id, sender_name, user_id, single_message, reply)
            else:
                logger.info(f"回复包含记忆片段标记，已屏蔽发送给用户 {user_id}。")
    
    except Exception as e:
        logger.error(f"直接处理消息失败 (用户: {user_id}): {str(e)}")
    
    finally:
        with interrupt_reply_lock:
            if current_processing_user == user_id:
                current_processing_user = None


def process_user_messages(user_id):
    """处理指定用户的消息队列，包括可能的联网搜索。"""
    global can_send_messages, current_processing_user, processing_start_time
    
    # 获取打断配置
    enable_interrupt = get_dynamic_config('ENABLE_INTERRUPT_REPLY', True)
    
    # 如果开启了打断功能，检查是否需要打断当前正在处理的用户的回复
    if enable_interrupt:
        with interrupt_reply_lock:
            if current_processing_user is not None and current_processing_user != user_id:
                logger.info(f"检测到新消息，用户 {current_processing_user} 的回复将被中断")
                # 标记当前处理结束（实际中断需要在发送时检查）
            # 标记开始处理此用户
            current_processing_user = user_id
            processing_start_time = time.time()
    ...
    if enable_interrupt:
        with queue_lock:
            has_new_message = user_id in user_queues and len(user_queues[user_id]['messages']) > 0
        
        if has_new_message:
            logger.info(f"检测到新消息，中断当前回复处理")
            # 合并新旧消息一起处理
            new_messages = user_queues.get(user_id, {}).get('messages', [])
...
"""
        ...
        if enable_interrupt:
            with interrupt_reply_lock:
                if is_interrupted:
                    logger.info(f"AI思考阶段检测到中断，停止处理用户 {user_id} 的消息")
                    with queue_lock:
                        if current_processing_user == user_id:
                            current_processing_user = None
                    return
...
def send_reply(user_id, sender_name, username, original_merged_message, reply, is_system_message=False):
    ...
    """
    global is_sending_message, current_processing_user, is_interrupted
    ...
    if enable_interrupt:
        with interrupt_reply_lock:
            # 检查中断标志
            if is_interrupted:
                logger.info(f"消息发送被中断，跳过向 {user_id} 发送")
                return
            # 检查是否是当前正在处理的用户
            is_processing = (current_processing_user == user_id)
        
        if is_processing:
            # 检查是否正在处理特殊操作（不打断）
            if image_generation_in_progress:
                logger.info(f"图片生成进行中，不打断当前回复")
            elif is_handling_reminder:
                logger.info(f"正在处理提醒操作，不打断当前回复")
            else:
                with queue_lock:
                    if user_id in user_queues and len(user_queues[user_id]['messages']) > 0:
                        has_real_user_message = False
                        for msg in user_queues[user_id]['messages']:
                            if "[20" in msg and "属性: self" not in msg and "用户 \"self\"" not in msg:
                                has_real_user_message = True
                                break

                        if has_real_user_message:
                            logger.info(f"用户 {user_id} 队列中有新用户消息，打断当前回复")
                            return
    ...
    try:
        ...
        # --- 语音生成逻辑 ---
        voice_path = None
        logger.info(f"[TTS] ENABLE_GPT_SOVITS_TTS={ENABLE_GPT_SOVITS_TTS}, tts_service={tts_service is not None}, is_system_message={is_system_message}")
        if ENABLE_GPT_SOVITS_TTS and tts_service and not is_system_message:
            try:
                clean_text = tts_service._clear_tts_text(reply)
                logger.info(f"[TTS] 清洗后文本: {clean_text[:50]}...")
                if clean_text:
                    logger.info(f"正在生成语音: {clean_text[:30]}...")
                    voice_path = tts_service.generate_audio(clean_text)
                    logger.info(f"[TTS] 生成结果: {voice_path}")
                    if voice_path:
        ...
        # 将语音消息添加到队列末尾（在所有文本之后）
        if voice_path:
            message_actions.append(('voice', voice_path))
            logger.info(f"[TTS] 已添加语音到消息队列: {voice_path}")

        logger.info(f"[TTS] message_actions长度: {len(message_actions)}, 包含语音: {any(a[0] == 'voice' for a in message_actions)}")
        ...
        for idx, (action_type, content) in enumerate(message_actions):
            if action_type == 'emoji':
                ...
                for attempt in range(3):
                    try:
                        if wx.SendFiles(filepath=content, who=user_id):
                            logger.info(f"已向 {user_id} 发送语音")
                            success = True
                            break
                        else:
                            logger.warning(f"发送语音失败，尝试第 {attempt + 1} 次")
                    except Exception as e:
                        logger.warning(f"发送语音异常，尝试第 {attempt + 1} 次: {str(e)}")

                    if attempt < 2:
                        time.sleep(0.5)

                # 发送后清理语音文件
                if success and tts_service:
                    try:
                        asyncio.run(async_delay())
                        tts_service.del_audio_file(content)
                    except:
                        pass

                if not success:
                    logger.error(f"语音发送失败，已重试3次")
                else:
...
def generate_image(prompt: str) -> Optional[str]:
    ...
    if not ENABLE_IMAGE_GENERATION:
        logger.warning("图片生成功能未启用")
        return None
    
    if not prompt:
        logger.warning("图片生成提示为空")
        return None
    ...
    try:
        ...
        if "data" in result and len(result["data"]) > 0:
            ...
            if image_url:
                ...
                temp_dir = os.path.join(root_dir, "Memory_Temp")
                os.makedirs(temp_dir, exist_ok=True)
                image_path = os.path.join(temp_dir, f"gen_image_{timestamp}.png")
        ...
        logger.error(f"图片生成返回格式异常: {result}")
        return None
        ...
        try:
            if e.response is not None:
                ...
                logger.error(f"API错误响应内容: {error_detail}")
        except Exception:
            pass
        logger.error(f"图片生成HTTP错误: {status_code} - {str(e)}")
        if status_code == 500:
            logger.error("API服务端内部错误，建议稍后重试或更换API服务")
            if "content_policy" in error_detail.lower() or "filtered" in error_detail.lower():
                logger.error("可能是prompt触发了内容过滤，请尝试更换描述")
        elif status_code == 401:
            logger.error("API认证失败，请检查API Key是否正确")
...
def send_generated_image(user_id: str, image_path: str) -> bool:
    """发送生成的图片给用户"""
    ...
    for attempt in range(3):
        try:
            if wx.SendFiles(filepath=image_path, who=user_id):
                logger.info(f"已向 {user_id} 发送生成的图片")
                success = True
                break
            else:
                logger.warning(f"发送图片失败，尝试第 {attempt + 1} 次")
        except Exception as e:
            logger.warning(f"发送图片异常，尝试第 {attempt + 1} 次: {str(e)}")
    ...
    if success:
        try:
            ...
            logger.info(f"已清理临时图片文件: {image_path}")
        except Exception as e:
            logger.warning(f"清理临时图片文件失败: {str(e)}")
    
    return success

def clean_old_generated_images():
    """清理Memory_Temp目录中超过1小时的图片文件"""
    try:
        temp_dir = os.path.join(root_dir, "Memory_Temp")
        ...
        for filename in os.listdir(temp_dir):
            if filename.startswith("gen_image_") and filename.endswith(".png"):
                file_path = os.path.join(temp_dir, filename)
                try:
                    file_age = current_time - os.path.getmtime(file_path)
                    if file_age > 3600:
                        os.remove(file_path)
                        logger.info(f"已清理过期图片: {filename}")
                except Exception as e:
                    logger.warning(f"清理图片失败 {filename}: {e}")
    except Exception as e:
        logger.error(f"清理过期图片出错: {e}")
...
def summarize_and_save(user_id, skip_check=False):
    ...
    try:
        ...
        base_summary_prompt = f"请以{prompt_name}的视角，用中文总结与{user_id}的对话，提取重要信息总结为一段话作为记忆片段（直接回复一段话）：\n{full_logs}"

        # 追加自定义提示词（如果启用且使用辅助模型）
        if ENABLE_ASSISTANT_CUSTOM_PROMPT and ASSISTANT_CUSTOM_PROMPT and USE_ASSISTANT_FOR_MEMORY_SUMMARY and ENABLE_ASSISTANT_MODEL:
        ...
        # --- 评估重要性 ---
        base_importance_prompt = f"为以下记忆的重要性评分（1-5，直接回复数字）：\n{summary}"

        # 追加自定义提示词（如果启用且使用辅助模型）
        if ENABLE_ASSISTANT_CUSTOM_PROMPT and ASSISTANT_CUSTOM_PROMPT and USE_ASSISTANT_FOR_MEMORY_SUMMARY and ENABLE_ASSISTANT_MODEL:
            importance_prompt = base_importance_prompt + "\n" + ASSISTANT_CUSTOM_PROMPT
        else:
            importance_prompt = base_importance_prompt
...
def try_handle_reminder_request(message_content, user_id):
    """
    统一处理提醒请求：通过AI判断是添加提醒还是删除提醒。
    返回True表示已处理完毕，否则返回False继续其他处理。
    """
    logger.debug(f"尝试处理提醒请求，用户 {user_id}: '{message_content}'")
    
    # 快速过滤：只有包含明确提醒意图关键词的消息才进一步处理
    add_keywords = ["提醒我", "设置提醒", "定时提醒", "每天", "每日", "几点", "分钟后", "小时后", "叫醒", "叫我", "闹钟", "通知我", "叫我起床", "叫我起床"]
    delete_keywords = ["删除提醒", "取消提醒", "删掉提醒", "不要提醒", "停止提醒", "撤销提醒", "清除提醒", "取消所有", "不需要叫", "不睡觉了", "不用叫了", "取消起床", "不要叫我", "不用提醒"]
    
    has_add_intent = any(kw in message_content for kw in add_keywords)
    has_delete_intent = any(kw in message_content for kw in delete_keywords)
    
    # 如果没有任何提醒关键词，直接返回False
    if not has_add_intent and not has_delete_intent:
        return False
    
    # 如果有删除关键词，优先处理删除
    if has_delete_intent:
        return try_delete_reminder(message_content, user_id)
    
    # 构建判断提示
    classification_prompt = f"""你是一个提醒意图分类器。请分析用户的消息是想要添加提醒还是删除提醒。

用户消息："{message_content}"

判断规则：
1. 如果用户想添加/设置一个新的提醒（包括定时提醒、每日提醒、几分钟后提醒等），返回：{{"intent": "add"}}
2. 如果用户想删除/取消/撤销已有的提醒，返回：{{"intent": "delete"}}
3. 如果用户只是在问提醒相关的事情、感谢提醒、或者不是真正的提醒请求，返回：{{"intent": "none"}}
...
"""
    ...
    try:
        client = OpenAI(api_key=ASSISTANT_API_KEY, base_url=ASSISTANT_BASE_URL)
        response = client.chat.completions.create(
            model=ASSISTANT_MODEL,
        ...
        logger.warning(f"AI判断提醒意图失败: {e}")
        intent = 'none'
    
    logger.info(f"提醒意图判断结果: {intent}，原始消息: {message_content[:50]}...")
    
    if intent == 'add':
        # 添加提醒
        return try_parse_and_set_reminder(message_content, user_id)
    elif intent == 'delete':
        # 删除提醒
        return try_delete_reminder(message_content, user_id)
    else:
...
def try_delete_reminder(message_content, user_id):
    """
    尝试删除用户的提醒。
    通过AI解析用户想删除的提醒内容，然后从列表中删除匹配的提醒。
    返回True表示处理完毕，否则返回False。
    """
    global is_handling_reminder
    is_handling_reminder = True
    logger.debug(f"尝试为用户 {user_id} 删除提醒: '{message_content}'")
    
    try:
        json_path = os.path.join(root_dir, 'recurring_reminders.json')
        if not os.path.exists(json_path):
            ai_prompt = f"""用户刚才说："{message_content}"
现在需要删除提醒，但发现用户没有任何提醒。
请根据这个情况，以你的角色身份生成回复。"""
            reply = get_deepseek_response(ai_prompt, user_id, store_context=True)
            send_reply(user_id, user_id, user_id, "[删除提醒]", reply if reply else "你还没有设置任何提醒哦~")
            is_handling_reminder = False
            return True
        ...
        ai_prompt = f"""用户刚才说："{message_content}"
现在需要删除提醒，但读取提醒列表时出错了。
请根据这个情况，以你的角色身份生成回复。"""
        reply = get_deepseek_response(ai_prompt, user_id, store_context=True)
        send_reply(user_id, user_id, user_id, "[删除提醒]", reply if reply else "读取提醒列表失败了呢...")
        is_handling_reminder = False
        return True
...
请根据这个情况，以你的角色身份生成回复。"""
        reply = get_deepseek_response(ai_prompt, user_id, store_context=True)
        send_reply(user_id, user_id, user_id, "[删除提醒]", reply if reply else "你还没有设置过提醒呢~")
        is_handling_reminder = False
        return True
    
    # 提取要删除的内容关键词
    delete_prompt = f"""请分析用户的删除提醒请求，提取用户想删除的提醒内容关键词。
用户消息："{message_content}"
当前用户的提醒列表：
{chr(10).join([f"- {r.get('content', '')}" for r in user_reminders])}

请返回JSON格式：
{{"delete_keyword": "用户想删除的提醒内容关键词"}}
如果用户想删除所有提醒，请返回 {{"delete_keyword": "全部"}}
如果无法确定，请返回 {{"delete_keyword": null}}
"""
    ...
    try:
        ...
        if json_match:
            ...
            delete_keyword = delete_data.get('delete_keyword')
        else:
            delete_keyword = message_content.replace("删除提醒", "").replace("取消提醒", "").strip()
    except Exception as e:
        logger.warning(f"AI解析删除关键词失败: {e}")
        delete_keyword = message_content.replace("删除提醒", "").replace("cancel提醒", "").strip()
    
    logger.info(f"删除提醒解析结果: delete_keyword='{delete_keyword}'")
    
    if not delete_keyword or delete_keyword == "null":
        reminder_list = "\n".join([f"- {r.get('content', '')}" for r in user_reminders])
        ai_prompt = f"""用户刚才说："{message_content}"
用户的提醒列表：{reminder_list}
请根据这个情况，以你的角色身份生成回复。
注意：用自然、像朋友聊天的语气，不要出现具体的时间数字。"""
        reply = get_deepseek_response(ai_prompt, user_id, store_context=True)
        send_reply(user_id, user_id, user_id, "[删除提醒]", reply if reply else f"你有以下提醒：\n{reminder_list}\n请告诉我你想删除哪个？")
        is_handling_reminder = False
        return True
    
    # 删除匹配的提醒
    original_count = len(reminders)
    logger.info(f"删除提醒: original_count={original_count}, delete_keyword='{delete_keyword}'")
    
    if delete_keyword == "全部" or delete_keyword == "all":
        reminders = [r for r in reminders if r.get('user_id') != user_id]
        deleted_count = original_count - len(reminders)
        logger.info(f"删除全部提醒: deleted_count={deleted_count}")
        save_result = save_recurring_reminders_to_file(reminders)
        ai_prompt = f"""用户说："{message_content}"
提醒删除成功，共删除了 {deleted_count} 条提醒。
请根据这个情况，以你的角色身份生成回复。
注意：用自然、像朋友聊天的语气，像真人一样回复。"""
        reply = get_deepseek_response(ai_prompt, user_id, store_context=True)
        send_reply(user_id, user_id, user_id, "[删除提醒]", reply if reply else f"好的，已经删除了你设置的 {deleted_count} 条提醒~")
        is_handling_reminder = False
        return True
    
    reminders = [r for r in reminders if r.get('user_id') != user_id or delete_keyword.lower() not in r.get('content', '').lower()]
    deleted_count = original_count - len(reminders)
    logger.info(f"关键词匹配删除: deleted_count={deleted_count}")
    
    if deleted_count > 0:
        save_result = save_recurring_reminders_to_file(reminders)
        ai_prompt = f"""用户说："{message_content}"
删除的关键词是：「{delete_keyword}」
成功删除了 {deleted_count} 条包含该关键词的提醒。
请根据这个情况，以你的角色身份生成回复。
注意：用自然、像朋友聊天的语气，像真人一样回复。"""
        reply = get_deepseek_response(ai_prompt, user_id, store_context=True)
        send_reply(user_id, user_id, user_id, "[删除提醒]", reply if reply else f"好的，已帮你删除包含「{delete_keyword}」的提醒~")
    else:
        reminder_list = "\n".join([f"- {r.get('content', '')}" for r in user_reminders])
        ai_prompt = f"""用户说："{message_content}"
想删除的关键词是：「{delete_keyword}」
但用户的提醒列表中没有匹配的提醒：{reminder_list}
请根据这个情况，以你的角色身份生成回复。
注意：用自然、像朋友聊天的语气，像真人一样回复。"""
        reply = get_deepseek_response(ai_prompt, user_id, store_context=True)
        send_reply(user_id, user_id, user_id, "[删除提醒]", reply if reply else f"没找到包含「{delete_keyword}」的提醒呢...你的提醒列表：\n{reminder_list}")
    
    is_handling_reminder = False
    return True


def save_recurring_reminders_to_file(reminders):
    """保存提醒到文件"""
    json_path = os.path.join(root_dir, 'recurring_reminders.json')
    try:
...
def try_parse_and_set_reminder(message_content, user_id):
    ...
    """
    global next_timer_id, is_handling_reminder
    is_handling_reminder = True
...
请用自然、像朋友聊天一样的语气回复用户。注意：
1. 按你的人物设定回复，不要机械地描述时间
2. 不要出现具体的年月日时分秒，用自然的表达方式，比如"过一会"、"等下"、"马上"等
3. 语气要像真人一样生动、亲切"""
...
请用自然、像朋友聊天一样的语气回复用户。注意：
1. 按你的人物设定回复
2. 不要直接说具体的年月日时分秒，用更自然的表达方式，比如"到那时候"、"那个时间点"、"到时候"等
3. 语气要像真人一样生动、亲切"""
                # 使用格式化后的时间发送给用户
...
请用自然、像朋友聊天一样的语气回复用户。注意：
1. 按你的人物设定回复
2. 不要说"每天xx:xx"这样的机械表达，用更自然的方式比如"每天那时候"、"每天那个点"等
3. 语气要像真人一样生动、亲切"""
...
# --- 检测是否需要生成图片的函数（主模型判断） ---
def needs_image_generation(message: str, user_id: str) -> bool:
    """
    使用 AI 模型判断用户消息是否需要生成图片。
    根据 IMAGE_DETECTION_USE_ASSISTANT 配置选择辅助模型或主模型。
    ...
    返回:
        bool: 如果需要生成图片返回 True，否则返回 False。
    """
    if not ENABLE_IMAGE_GENERATION:
        return False

    # 构建用于检测的提示词（只做判断，区分自拍和画图）
    detection_prompt = f"""你是一个文本分类器，任务是判断用户消息是需要生成图片的哪种类型。

请判断以下用户消息是需要生成图片还是只是发送已有图片。
用户消息："{message}"

判断规则：
1. 如果用户要求"画一个..."、"生成一张..."、"生成一个..."、"创作..."、"给我画"等主动要求生成新图片，是画图
2. 如果用户要求"自拍"、"看看你的照片"、"给我看看你的照片"、"看你照片"、"你的照片"等要求展示或生成自己的照片，是自拍
3. 如果用户发送图片给我、分享图片、让我看图片等（不是要求生成新图片），不需要生成图片
4. 如果用户的消息是普通聊天、询问问题、闲聊、夸图片好看等，不需要生成图片

请严格按照以下格式回答：
- 如果是画图类型，请只回答：画图
- 如果是自拍类型，请只回答：自拍
- 如果不需要生成图片，请只回答：不需要画图

不要输出任何其他内容。
"""

    try:
        # 根据配置选择使用辅助模型还是主模型进行画图检测
        if IMAGE_DETECTION_USE_ASSISTANT and ENABLE_ASSISTANT_MODEL:
            logger.info(f"向辅助模型发送画图检测请求，用户: {user_id}，消息: '{message[:50]}...'")
            response = get_assistant_response(detection_prompt, f"image_detection_{user_id}")
            model_label = "辅助模型"
        else:
            logger.info(f"向主 AI 发送画图检测请求，用户: {user_id}，消息: '{message[:50]}...'")
            response = get_deepseek_response(detection_prompt, user_id=f"image_detection_{user_id}", store_context=False)
            model_label = "主模型"
        ...
        logger.info(f"{model_label}画图检测响应: '{cleaned_response[:200]}'")

        if "不需要画图" in cleaned_response:
            logger.info(f"用户 {user_id} 的消息不需要画图。")
            return ("不需要画图", None)
        elif "自拍" in cleaned_response:
            logger.info(f"检测到用户 {user_id} 的消息需要自拍，将使用自拍提示词。")
            return ("自拍", None)
        elif "画图" in cleaned_response:
            logger.info(f"检测到用户 {user_id} 的消息需要画图，将使用消息原文作为提示词。")
            return ("画图", message)
        else:
            logger.warning(f"无法解析画图检测响应，用户: {user_id}，响应: '{cleaned_response[:200]}'")
            return ("不需要画图", None)

    except Exception as e:
        logger.error(f"画图检测失败，用户: {user_id}，错误: {e}", exc_info=True)
        return ("不需要画图", None)  # 出错时默认不需要画图
...
"""
    # 追加自定义提示词（如果启用）
    if ENABLE_ASSISTANT_CUSTOM_PROMPT and ASSISTANT_CUSTOM_PROMPT:
        detection_prompt = base_detection_prompt + "\n" + ASSISTANT_CUSTOM_PROMPT
    else:
        detection_prompt = base_detection_prompt
...
def main():
    try:
        ...
        #     wx.Show()
        # except:
        #     logger.error(f"无法初始化微信接口，请确保您安装的是微信3.9版本，并且已经登录！")
        #     exit(1)

        for user_name in user_names:
            if user_name == ROBOT_WX_NAME:
                logger.error(f"您填写的用户列表中包含自己登录的微信昵称，请删除后再试！")
                exit(1)
            ListenChat = wx.AddListenChat(nickname=user_name, callback=message_listener)
            if ListenChat:
                logger.info(f"成功添加监听用户{ListenChat}")
            else:
                logger.error(f"添加监听用户{user_name}失败，请确保您在用户列表填写的微信昵称/备注与实际完全匹配，并且不要包含表情符号和特殊符号，注意填写的不是自己登录的微信昵称!")
                exit(1)
